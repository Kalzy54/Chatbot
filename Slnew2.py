...
def _sidebar_kb():
    with st.expander("📤 Knowledge Base", expanded=False):
        ...
        files = st.file_uploader("Files", type=["txt", "pdf", "md"], accept_multiple_files=True, label_visibility="collapsed")
        if files and st.button("Process & Index", use_container_width=True):
            ...
            with st.spinner("Reindexing…"):
                try:
                    ...
                    st.success("✅ Index updated") if r.status_code == 200 else st.error("Reload failed")
                except Exception as e:
...
def render_sidebar():
    with st.sidebar:
        ...
        with st.expander("ℹ️ About", expanded=False):
            st.markdown("""
**LibraryChat** — AI library assistant
...
- Streamlit
        ...
        st.markdown("---")
        st.markdown(
            '<p style="text-align:center;font-size:11px;color:var(--text-tertiary);">LibraryChat v2.0</p>',
            unsafe_allow_html=True,
        )
...
def view_welcome():
    ...
    <div class="hero">
        ...
        <h1>Welcome to <span class="gradient-text">LibraryChat</span></h1>
